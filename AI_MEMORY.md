...
### [2026-08-25 — Замена эмодзи на SVG Lucide и фильтрация 1 Robux артефактов как OFF-SALE]
- **Замена эмодзи на Lucide-иконки:**
  - В `GroupStoreModal.tsx` селект сортировки очищен от эмодзи (`🆕`, `🔼`, `🔽`) и оснащен минималистичной иконкой `<ArrowUpDown />`.
  - Кнопка массовой загрузки переведена с эмодзи `⚡` на аккуратную иконку `<Zap />`.
  - В `FavoritesDrawer.tsx` и `useFavorites.ts` эмодзи `❤️` заменены на SVG `<Heart />` и чистые тосты.
- **Определение 1 R$ как OFF-SALE:**
  - В Roblox минимальная цена для классической одежды (Рубашки / Штаны) составляет 5 R$ (для футболок 2 R$). Значение `price: 1` или `priceStatus: "Off Sale"` в API — это артефакт старых/снятых с продажи вещей (как у `Gucci`).
  - `robloxService.ts` и карточки интерфейса теперь гарантированно распознают `price <= 1` (без Free-статуса) и `priceStatus === 'Off Sale'` как **`OFF-SALE`** с `price: null` и `isForSale: false`.

### [2026-08-25 — Восстановление реальных названий и цен вещей в Магазине Группы (search/items/details)]
- **Проблема «Item #... / OFF-SALE на всех вещах» (зачем и как фиксили):**
  - При запросе магазина через `/v1/search/items` Roblox возвращал только минимальный массив `{ id, itemType }` без полей `name` и `price`. В результате все вещи отображались как `Item #110694584437494` и считались `OFF-SALE`.
  - Кроме того, `POST /v1/catalog/items/details` требовал `x-csrf-token` рукопожатия, а параллельный опрос одиночных `/v2/assets/details` в цикле триггерил 429 rate limit.
  - **Решение:**
    1. Перевели `getGroupStore` на прямой GET эндпоинт **`https://catalog.roblox.com/v1/search/items/details`** (с автоматическим фоллбеком на `catalog.roproxy.com`). Этот эндпоинт в одном GET запросе возвращает полные подлинные названия (*Miracles in the Sleeping Area*, *Suffocation*, *fashionable*, *67*), настоящие цены в робуксах (5 R$, 6 R$), категории и актуальные статусы продажи.
    2. Добавлен `postWithCsrf` с автоматической обработкой 403 CSRF-токена для пакетного разрешения каталога.
    3. Разделены `On-Sale` и `Off-Sale` фильтры с точным определением `isForSale` / `isOffSale` по `priceStatus` и наличию цены.
    4. Превью загружаются пакетно через `/v1/batch` (до 100 за 1 запрос).

### [2026-08-25 — RoProxy Fallback для работы через VPN и Магазин Групп]
- **VPN-иммунитет (RoProxy Failover):**
  - При использовании VPN адреса часто ловят `429 Too Many Requests` от официальных серверов Roblox.
  - В `robloxService.ts` добавлен автоматический каскадный фоллбек: при таймауте или 429 от `*.roblox.com` запросы мгновенно перенаправляются на `*.roproxy.com` (каталог, экономика, аватарки, превью).
  - Строгий клампинг `limit` под требования Roblox API (`10, 28, 30, 50, 60, 100, 120`), исключающий ошибку 400 Bad Request.
  - Увеличен базовый лимит до 100 вещей за раз, чтобы показывать **абсолютно все вещи группы** (все 56+ предметов сразу, включая снятые с продажи и находящиеся в продаже).
  - В `GroupStoreModal.tsx` добавлена кнопка `LOAD ALL STORE ITEMS ⚡` для автоматической пакетной загрузки сотен вещей подряд в один клик.
- **Магазин групп и Избранное:**
  - `GroupStoreModal.tsx`: адаптивная сетка (2-6 колонок), двухстрочные названия, фильтры *All / On Sale / Off-Sale*, сортировка, пагинация по курсору, кнопка *Refresh/Retry*.
  - `FavoritesDrawer.tsx` + `useFavorites.ts`: вишлист с подсчётом общей суммы в Robux, копированием всех ID и синхронизацией.

### [2026-08-25 — Полное удаление Discord UI и интеграции по запросу пользователя]
- **Очистка сайта от Discord:** Полностью удалена кнопка «Discord подключён / Connect Discord», кнопка «Сменить», плашки уведомлений и аккордеон с инструкцией по Discord боту из `CopiedGroupsFolder.tsx` и `Stage2Inspector.tsx`.
- **Локальная папка:** `useCopiedGroupsFolder.ts` и `CopiedGroupsFolder.tsx` теперь сфокусированы на чистом локальном хранении скопированных групп, быстром копировании названий, переходе по ссылкам и встроенной проверке обновлений (`Check new`).

### [2026-08-25 — Полный авто-синк метаданных групп, фикс 0 members и авто-подписка]
- **Проблема «0 members / Group #ID» и пропуск новых групп (зачем):** При вызове `/folder` бот слал 35 параллельных запросов к Roblox API, ловил 429 и ставил `0 members`. Кроме того, скопированные без явного входа в Discord группы не привязывались к подписчикам и отсекались фильтром. **Как фиксили:**
  - `folderStore.ts:11-340`: добавлено постоянное хранение `groupMeta` (название, участники, аватарка) прямо в Redis/файле.
  - `folderStore.ts:240-255`: `track()` теперь автоматически привязывает группу к активному Discord пользователю, даже если запрос пришел без явного токена.
  - `server/index.ts:375-470`: `/api/folder/sync` и `sync-bulk` принимают массив `groups` с полными именами и участниками и сохраняют в `groupMeta`.
  - `src/hooks/useCopiedGroupsFolder.ts:50-115`: сайт при копировании любой группы сразу отправляет её имя и количество участников на бэкенд.
  - `server/robloxService.ts:510`: добавлен 30-минутный кэш `groupInfoCache`.
  - `server/discordBot.ts:380-450`: `/folder` берет имена из `savedMetas` мгновенно, недостающие тянет пачками по 5, и автоматически привязывает все найденные группы к Discord DM пользователя.
  - `server/discordBot.ts:40-60`: ротация увеличена до 12 групп за тик (полный круг 35-40 групп за ~3 тика).
